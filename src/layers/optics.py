...
class DifferentiableOptics(nn.Module):
    def __init__(self,
        ...
        min_lambda = 470e-9
        print(f"[Optics Check] Diffraction Spot Size approx: {1.22*min_lambda*f_number*1e6:.2f} um")

        # --- Initialize Zernike  (保留上一版的逻辑) ---
        with torch.no_grad():
            self.zernike_0.zero_()
            self.zernike_90.zero_()
            val_astig = 0.4 * 1e-6 # 0.3 micron
            val_coma  = 0.4 * 1e-6 # 0.3 micron
            if n_zernike > 6:
        ...
        # --- 预计算 ---
        basis_numpy, aperture_numpy, r2_phys = self._precompute_grids(n_zernike, pupil_grid_size)
        ...
        # 波长: (1, 3, 1, 1) 用于广播
        self.register_buffer('wavelengths', torch.tensor([630e-9, 550e-9, 470e-9]).view(1, 3, 1, 1))
    ...
    def _compute_psf(self, coeffs, extra_phase=None):
        ...
        for i in range(3): 
            ...
            fft_total_width = (self.wavelengths[0, i, 0, 0] * f_m * self.pupil_grid_size) / self.pupil_diam
            sensor_total_width = self.pixel_size * self.fov_pixels
            s = sensor_total_width / fft_total_width
            
            grid_1d = torch.linspace(-s, s, high_res_fov, device=psf_raw.device)
            ...
            sampled_high_res = F.grid_sample(psf_in, grid, mode='bilinear', align_corners=False)
            psf_channels.append(sampled_high_res)

        psf_high_res = torch.cat(psf_channels, dim=1)
        psf_final = F.avg_pool2d(psf_high_res, kernel_size=self.oversample_factor, stride=self.oversample_factor)
    ...
    def forward(self, d_obj, current_focus_dist_0, current_focus_dist_90):
        ...
        if phase_defocus_0.ndim > 3 and coeffs_0.ndim == 1:
             ...
             coeffs_0 = coeffs_0.unsqueeze(0).repeat(phase_defocus_0.shape[0], 1)
        ...
        if phase_defocus_90.ndim > 3 and coeffs_90.ndim == 1:
             coeffs_90 = coeffs_90.unsqueeze(0).repeat(phase_defocus_90.shape[0], 1)
