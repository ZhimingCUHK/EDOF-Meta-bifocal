...
class SCM(nn.Module):
    def __init__(self, out_plane):
        ...
        self.main = nn.Sequential(
            BasicConv(6, out_plane//4, kernel_size=3, stride=1, relu=True),
            BasicConv(out_plane // 4, out_plane // 2, kernel_size=1, stride=1, relu=True),
            BasicConv(out_plane // 2, out_plane // 2, kernel_size=3, stride=1, relu=True),
            BasicConv(out_plane // 2, out_plane-6, kernel_size=1, stride=1, relu=True)
        )
...
class MIMOUNet(nn.Module):
    ...
    def forward(self, x):
        # --- 修复：生成 3 通道参考图用于残差连接 ---
        # 输入 x 是 [B, 6, H, W]，我们取前后两张图的平均值作为 3 通道基准
        x_ref = (x[:, :3] + x[:, 3:]) / 2
        x_ref_2 = F.interpolate(x_ref, scale_factor=0.5)
        x_ref_4 = F.interpolate(x_ref_2, scale_factor=0.5)
        # ---------------------------------------
        ...
        outputs.append(z_+x_ref_4) # 使用 3 通道的 x_ref_4
        ...
        outputs.append(z_+x_ref_2) # 使用 3 通道的 x_ref_2

        z = torch.cat([z, res1], dim=1)
        z = self.Convs[1](z)
        z = self.Decoder[2](z)
        z = self.feat_extract[5](z)
        outputs.append(z+x_ref)    # 使用 3 通道的 x_ref
